...
static void e164_test_load_plan_file(void **state)
{
    ...
    itu_t_e164_set_value(&e164, "080001010101");
    assert_string_equal("558000101010", e164.value);
    assert_int_equal(ITU_T_E164_NUMBER_KIND_TOLL_FREE, itu_t_e164_get_number_kind(&e164));
    ...
    assert_int_equal(ITU_T_E164_NUMBER_KIND_REGULAR, itu_t_e164_get_number_kind(&e164));
...
static void e164_test_dialing_international_incremental_limit(void **state)
{
    (void) state;
    itu_t_e164_t e164;
    itu_t_e164_context_t context = {
    ...
    char input[32] = "";
    const char *digits = "00218001234567890123456";
    size_t i;
    ...
    for(i = 0; digits[i] != 0; i++) {
        ...
        input[len + 1] = 0;
        itu_t_e164_set_value(&e164, input);
        if(strcmp(e164.value, previous_value) == 0 && previous_value[0] != 0 &&
    ...
    assert_string_equal("0021800123456789012", input);
    assert_string_equal("800123456789012", e164.value);
    assert_int_equal(15, e164.pos);

    itu_t_e164_reset_plan();
}

static void e164_test_dialing_omitted_carrier_incremental_limit(void **state)
{
    (void) state;
    itu_t_e164_t e164;
    itu_t_e164_context_t context = {
    ...
    char input[32] = "";
    const char *digits = "00551998259222222222222222";
    size_t i;
